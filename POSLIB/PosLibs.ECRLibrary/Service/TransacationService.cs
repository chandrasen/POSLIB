    ...
    public  class TransacationService
    {
        ...
        public void doTransaction(string inputReqBody, int transactionType, TransactionListener transactionListener)
        {
                ...
                else
                {
                    if (configdata.connectionMode == "TCP/IP")
                    {
                        ...
                        try
                        {
                            ...
                            {
                                isSend = conobj.sendTcpIpTxnData(transactionRequestbody);
                                if (isSend == true)
                                {
                    ...
                    else if (configdata.connectionMode == "COM")
                    {
                        if(isNetworkAvailabe())
                        {
                            ...
                            Console.WriteLine("Selected ConnectionMode" + configdata.connectionMode);
                            //conobj.sendData(transactionRequestbody);
                            conobj.sendCOMTXNData(transactionRequestbody);
                            Array.Clear(buffer, 0, buffer.Length);
                            responseString = conobj.receiveCOMTxnrep();
                            Console.WriteLine("COM Transaction Response:" + responseString);
                        }
            ...
            if (trasnlistener != null)
            {
                ...
                //if (transactionReponse.responseBody != null)
                //{
                //    trasnlistener.onSuccess(transactionReponse.responseBody);
                //}
            }
            else
            {
        ...
        public bool isNetworkAvailabe()
        {
            ...
            if (configdata != null)
            {
                if (isavilabe = conobj.isComDeviceConnected(configdata.commPortNumber))
                {
                    isavilabe = true;
                }
                else
